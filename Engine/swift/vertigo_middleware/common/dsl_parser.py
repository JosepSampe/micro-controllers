...
def parse(input_string):
    ...
    filters = map(lambda x: "".join(
        x), {'USER_NAME', 'ROLES'})  # Object Metadata
    filter_list = oneOf(filters)("filter")

    storlets = map(lambda x: "".join(
        x), {'COMPRESSION', 'FILTER_CONTENT', 'TRANSCODER', 'GREP'})  # Storlets
    storlets_list = oneOf(storlets)("storlet")
    json = Combine(MARK + WORD + MARK + COLON + MARK +
                   WORD + MARK + Optional(COMMA))
    ...
    """ MOVE, COPY COMMAND """
    move_copy = MOVECOPY + \
        obj("source") + TO + obj("destination") + \
        Optional(WHEN + condition)("condition")
...
rule_parsed = parse(
    "FOR data/out/sample_3mb.csv, data/sampe3_gb.csv DO SET COMPRESSION BY USER_NAME {'pepe':'gz','david':'zip'}, SET GREP BY ROLES {'admin':'^r'}")
...
""" MOVE OBJECT """
rule_parsed = parse(
    "MOVE data/out/sample_3mb.csv TO data/sampe3_gb.csv WHEN gets > 5")
print "\n--> Move test:"
