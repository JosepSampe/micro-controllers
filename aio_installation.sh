...
install_openstack_keystone(){
	...
	openstack project create --domain default --description "Vertigo Test Project" vertigo
	
	openstack user create --domain default --password $VERTIGO_TENANT_PASSWD vertigo
	
	openstack role add --project vertigo --user vertigo admin
	openstack role add --project vertigo --user vertigo ResellerAdmin
	openstack role add --domain default --user vertigo ResellerAdmin
	
	cat <<-EOF >> vertigo-openrc
	export OS_USERNAME=vertigo
	export OS_PASSWORD=$VERTIGO_TENANT_PASSWD
	export OS_PROJECT_NAME=vertigo
	export OS_USER_DOMAIN_NAME=Default
	export OS_PROJECT_DOMAIN_NAME=Default
